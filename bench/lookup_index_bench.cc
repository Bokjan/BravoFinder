...
int main(int argc, char** argv) {
  ...
  // Materialize the real (ident, arinc424_icao_code) list once, so index-build
  // timing does not include the FixedIdent->string cost.
  std::vector<std::pair<std::string, std::string>> keys;
  keys.reserve(V);
  for (const bf::FixedIdent& fi : idents) {
    keys.emplace_back(std::string(fi.IdentView()), std::string(fi.Arinc424IcaoCodeView()));
  }
  ...
  // ---- 2. Lookup workload: every real key, shuffled ----
  ...
  std::printf("sizeof: Ident=%zu FixedIdent=%zu FixedName8=%zu pair<FI,int>=%zu\n\n",
              sizeof(bf::Ident), sizeof(bf::FixedIdent), sizeof(bf::FixedName8),
              sizeof(vec_index[0]));
