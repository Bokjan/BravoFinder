...
std::string FindRoutesSchema() {
  ...
  return std::string(R"({"type":"object","properties":{)") + JKey(kKeyDeparture) +
         ...
         R"(:{"type":"array","items":{"type":"string"},"description":"ARINC 424 ICAO region codes (NOT airport identifiers), each matched as a prefix. Omit or leave empty for any region. Listing several names a region set in ONE rule, which is free at runtime and often necessary for precision: prefix \"Z\" covers ZM (Mongolia) and ZK (North Korea) besides China, so mainland China is the ten FIRs ZB,ZG,ZH,ZJ,ZL,ZP,ZS,ZU,ZW,ZY."},)" +
         JKey(kKeyDesignators) +
         R"(:{"type":"array","items":{"type":"string"},"description":"Airway designators, compared per the match field. Omit or leave empty for any designator. Concurrency names (A14-M1) are split first, so a rule hits when any of their designators matches."},)" +
         ...
         JKey(kKeyAction) +
         R"(:{"type":"string","enum":[)" + JKey(kActionBlock) + "," + JKey(kActionPenalize) + R"(],"description":"block excludes matching legs outright; penalize (the default) adds a soft cost, keeping them usable. Prefer penalize for bulk region rules: block is an irreversible connectivity break and can leave an airport that depends on a single airway with no route at all."},)" + JKey(kKeyPenaltyFraction) + R"(:{"type":"number","minimum":0,"description":"Soft penalty as a fraction of each leg's own length, used only when action is penalize. Defaults to )" + std::to_string(bf::kDefaultPenaltyFraction) + R"(, which pushes matched airways out of the optimal route while leaving them usable when no alternative exists. Larger values steer harder; there is no upper bound."}}}},)" + JKey(kKeyRandomSeed) +
