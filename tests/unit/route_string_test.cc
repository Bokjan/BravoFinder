...
TEST_CASE("a genuine airway change breaks the group at the hand-off fix", "[unit][route_string]") {
  std::vector<bf::RouteLeg> legs{
      Leg("A", "B", "V28"),
      Leg("B", "C", "V28-Y28"),
      Leg("C", "D", "Y28"),
...
TEST_CASE("multi-way concurrency intersects across all designators", "[unit][route_string]") {
  std::vector<bf::RouteLeg> legs{
...
TEST_CASE("DCT legs never fold and list every fix", "[unit][route_string]") {
  std::vector<bf::RouteLeg> legs{
      Leg("KJFK", "ROBER", "DCT"),
      Leg("ROBER", "MERIT", "Y28"),
      Leg("MERIT", "KBOS", "DCT"),
...
TEST_CASE("empty legs yield just the first point", "[unit][route_string]") {
  std::vector<bf::RouteLeg> legs;
  CHECK(bf::BuildRouteString("KJFK", legs) == "KJFK");
}

TEST_CASE("SplitDesignators drops empty segments", "[unit][route_string]") {
