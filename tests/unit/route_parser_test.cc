...
TEST_CASE("tokenize: collapses runs of whitespace and tabs", "[route_parser]") {
...
TEST_CASE("tokenize: empty and whitespace-only strings yield no tokens", "[route_parser]") {
  CHECK(bf::TokenizeRoute("").empty());
  CHECK(bf::TokenizeRoute("   \t\n ").empty());
}

TEST_CASE("tokenize: preserves IDENT/REGION and hyphenated designators", "[route_parser]") {
