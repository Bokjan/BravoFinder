...
> - `docs/` — public-facing docs: `CONTRIBUTING.md`, algorithm articles, and architecture pieces (the authority for "how it works now"; see `docs/README.md` for the index).
...
- The route engine is `libs/engine/` (`bravofinder`, alias `bf::bravofinder`) and has **no** JSON / network dependency. Query capabilities shared across transports and the CLI live in the top-level `libs/service/` (namespace **`bf::service`**, target `bf_service_lib`): the multi-cycle `NavDatabaseRegistry`, the typed query entries (`queries.h`), and the ten JSON-args handlers, each returning `HandlerResult{body, status}` (an HTTP-style code). Rendering (JSON + text) lives in `render.{h,cc}`; the handlers are a thin JSON-args adapter over the typed entries (`OutputFormat::kJson`), and the CLI calls the typed entries directly with the user's `--format` choice. `libs/service/` is an app-tier library (it pulls in rapidjson), kept out of `libs/engine/` so the engine stays JSON-/network-free.
- **License boundary**: `libs/engine/` is distributed under **LGPL-3.0-or-later** (see `libs/engine/LICENSE`); everything else is MIT. Do not introduce any GPL-only dependency into `libs/engine/` (it would force the whole engine to GPL), and keep its JSON-/network-free boundary. Because the engine is statically linked into the `bf` / `bf-http` / `bf-mcp` binaries, a release must ship `libs/engine/LICENSE` (+ `LICENSE.GPLv3`). The complete corresponding source for LGPL relinking is the matching git tag on GitHub (SDK packages ship headers + `.a` + license texts, not a separate engine source tarball).
- The hand-rolled HTTP/1.1 transport core lives in the top-level `libs/http_server/` (namespace **`bf::http_server`**, target `bf_http_server`): the connection state machine (`conn.{h,cc}`), TCP listener (`server.{h,cc}`), and threadpool offload (`work.{h,cc}`), plus the `RequestHandler` interface and the transport-local `WorkResult` (`transport.h`). It is **JSON-/query-neutral** — no dependency on `bf_service_lib` (mirroring how `libs/engine/` stays network-free); a consumer implements `RequestHandler` to give a request meaning. It is shared by the REST server and the MCP-over-HTTP transport. Its file is `transport.h`, **not** `dispatcher.h` — the MCP JSON-RPC dispatcher (`apps/mcp/dispatcher.h`) owns that name, and both dirs land on the include path when compiling `bf_mcp_lib`, so a same-named header would be ambiguous.
- Three **peer** consumers depend on `libs/service/`, never on each other: `apps/mcp` (`bf-mcp`, MCP over **stdio or HTTP** — projects `is_error = status >= 400`; `bf-mcp --transport stdio|http`, default `stdio`), `apps/http` (`bf-http`, **REST**+JSON — uses `status` directly), and `apps/cli` (`bf` — prints `body`, maps `status >= 400` to a non-zero exit code, uses the registry-free single-`NavDatabase` path). The MCP protocol core is the transport-neutral `Dispatcher` (`apps/mcp/dispatcher.{h,cc}`), shared by both MCP transports (stdio inlines `Dispatch`; HTTP offloads it); `stdio_runner.{h,cc}` and `mcp_http.{h,cc}` are the two thin transport shells. `apps/http` and `apps/mcp`'s http mode both build on `bf_http_server`. Add a query capability **once** in `bf::service`; all three consumers get it. **REST must stay** (the `BravoFinderWeb` Go gateway byte-proxies REST by path+method+`?cycle=` and never parses a body — do not replace it with pure MCP). Don't add an http↔mcp↔cli dependency.
- **HTTP offload minefield** (`libs/http_server/`, used by `apps/http` REST and `apps/mcp` HTTP): the 10–30 ms route compute must run on the libuv **threadpool** (`uv_queue_work`), never the loop thread. A work item holds a strong `shared_ptr<Connection>` so the connection survives a client disconnect mid-compute; the completion callback checks `IsAlive()` before writing, and workers never touch libuv handles. The consumer builds the offloaded work as a **copyable** `std::function<WorkResult()>` (own the parsed request via `shared_ptr<Document>` — a move-captured Document is not copyable and cannot go into `std::function`). SSE writes (`BeginStream`/`WriteEvent`, close-delimited open stream, no `Content-Length`; the `streaming_` flag stops `OnWriteDone` from resetting/closing) happen on the **loop thread**, not workers. Touching this concurrency → `ctest --preset tsan` (thread-safety contract).

## Thread-safety contract (must follow when touching concurrency)
...
- ① Program version (CMake `project VERSION` → `libs/engine/core/version.h.in`'s `kBravoFinderVersion` → `bf --version`). Shape is MAJOR.MINOR.PATCH but the policy is **not** strict SemVer: v3 is in active development with interfaces unfrozen, so MAJOR stays pinned at 3 (absent a major shift), a large new feature or a breaking fix bumps MINOR, and any other code change / bugfix / tweak bumps PATCH.
...
- **Two independent layers — do not conflate them.** Cache disk layout change → bump container `format_version` (②). Any C++ source change (`.cc`/`.h`) → bump program version (①) per the PATCH/MINOR rule above — **including pure refactors / file moves with no behavior change** (that's a PATCH). A **comment-only** change (touching no code, only comments) is exempt — it alters no compiled artifact, so it needs no version bump. Read-side / internal-only changes do not change the disk layout, so they **do not bump `format_version`** — but they still bump the **program version**. The "do not bump" here is scoped to `format_version` only; it never exempts a C++ change from the program-version PATCH bump.
- **Protective (poison) `format_version` bump — no layout change.** Also bump `format_version` when a write-side bug made an *older* version produce byte-valid, version-valid, but semantically wrong or incomplete files (they load fine and silently serve bad data — there is no other signal). Retiring the old version forces a rebuild. The bump invalidates *all* caches (the version is unified across loaders/sections), which is the accepted cost — a cheap forced rebuild beats silently serving a bad cache. The reason for each bump lives in the commit message of the bump (what the old version produced wrong, and which loader(s) it affected); do not duplicate the history as a comment on `kFormatVersion` — git is the single home for it.
...
ctest --preset tsan -j 32    # must run after any concurrency-related change (thread-safety contract)
```
