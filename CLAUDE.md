...
- The route engine is `lib/` (`bf3`, alias `bf::bravofinder3`) and has **no** JSON / network dependency. Query capabilities shared across transports live in `apps/query_core` (namespace **`bf::service`**): the multi-cycle `NavDatabaseRegistry` and the nine handlers, each returning `HandlerResult{body, status}` (an HTTP-style code).
- Two **peer** transports depend on `query_core`, never on each other: `apps/mcp_stdio` (`bf-mcp-stdio`, MCP over stdio — projects `is_error = status >= 400`) and `apps/http_server` (`bf-http`, HTTP+JSON via libuv + llhttp — uses `status` directly). Add a query capability **once** in `bf::service`; both transports get it. Don't add an http↔mcp dependency.
