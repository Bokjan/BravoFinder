...
- **License boundary**: `libs/engine/` is distributed under **LGPL-3.0-or-later** (see `libs/engine/LICENSE`); everything else is MIT. Do not introduce any GPL-only dependency into `libs/engine/` (it would force the whole engine to GPL), and keep its JSON-/network-free boundary. Because the engine is statically linked into the `bf` / `bf-http` / `bf-mcp` binaries, a release must ship `libs/engine/LICENSE` (+ `LICENSE.GPLv3`) and the complete corresponding source so recipients can relink a modified engine.
