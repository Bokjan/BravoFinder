...
- **[Procedure modeling and airway-network hookup](procedure-modeling.zh-CN.md)** — all 23 ARINC 424 / CIFP path terminators, real-SID/STAR airport hookup, junction-fix selection, multi-source K-shortest, DCT-to-IAF when no STAR exists, DCT-splice when published gates are off-network, semantic honesty about connection kinds.
