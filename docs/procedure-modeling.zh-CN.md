...
- **公布的入口/出口全不在航路上时，不再拿中段 fix 凑合**：早期若整个机场该侧没有一个接得进航路网的发布衔接点，会退而求其次，把程序途经的任意在网 fix 也暴露成候选（机场级 mid-join）。这能保住「还有 STAR/SID」的说法，但那些中段点往往已经贴着机场、高度也接近落地——典型是 ENGM→ESNZ 算出 `arr ≈ 0.5 NM` 这种门前一步的假进场。issue #30 起废掉这条退路：入口/出口接不进航路网时，改走 §6.1 的 **DCT 拼接**，从航图上真正的衔接点进程序，而不是在半路上抄近路。
...
有 STAR、且公布入口已接上航路网的机场，走 §4 的正常路径。若机场**明明有**公布的 STAR/SID，但那些入口/出口**全都接不进航路网**，不能再假装「没 STAR」去走本节的进近过渡，也不能再靠中段 fix 抄近路——那是 §6.1 的 DCT 拼接。

## 6.1 有 STAR/SID，但公布的入口/出口接不进航路网

§6 处理的是「机场根本没公布 STAR」。现实里还有另一类尴尬机场：航图上**有** STAR（或 SID），可是航图标明的衔接点——STAR 的 Initial Fix、SID 的末端 fix——每一个都能在导航数据里解析成顶点，却**接不进航路网**（到达侧没有入边，离场侧没有出边）。cycle 2601 / X-Plane 12 全量里，到达侧大约 30 家属于「入口不在航路上，但程序中段碰巧有在网点」（从前靠 mid-join 硬接，容易出现门前一步的假进场），另有约 77 家连中段也没有在网点（`BuildArrival` 直接空，引擎曾静默改走 §6 的进近，路线被标成 `terminal_transition`，具名 `--star` 反而报「找不到」——KASE 的 `HAREI1` 就是例子）。离场侧对称：约 6 家曾靠中段凑合、约 255 家出口全部接不进航路网。

issue #30 对这两类一并改道，核心是三句话：

1. **废止机场级 mid-join**——不再用「程序碰巧经过的在网 fix」冒充衔接点。
2. **用附近的在网代理点做搜索，但航路串里写出真正的衔接点**——搜索仍停在代理点 F（不改航路图、不加虚拟边）；生成航路串时在 F 与机场之间插入航图上的入口/出口，写成 `… F DCT ENTRY STAR ARR` 或 `DEP SID EXIT DCT F …`。这与 §6 相反：§6 里接不进航路网的 IAF **不会**出现在航路串里；这里的 ENTRY/EXIT **必须**写出来，否则串形在说谎。
3. **到达侧与进近放在一起比价，并略偏袒公布的 STAR**——STAR 拼接候选和进近候选进同一个 A\* 池。仅当本场确实存在可拼接的 STAR 时，才给进近的 seed 加一小笔偏好（实现里是可调的常量，大约 15 海里），避免在相差无几的距离上无故丢掉公布程序；纯 §6、根本没有 STAR 的机场不加这笔偏好，以免改变 KTVL / KMFR 的行为。代价完全相同时选 STAR。用户写了 `--star` / `--sid` 则只出对应程序，匹配失败照旧报错，绝不静默改走进近。

定价上，搜索用的 seed 仍是「代理点到衔接点的大圆距离 + 整段程序体」；对外展示的相位距离则按填表拆开：`arr` / `dep` 只计 STAR / SID 那一段程序体，衔接点之前那截 DCT 算进航路（enroute）——与 `ParseRoute` 按 `via` 拆分的规则一致。连接种类仍是 `kProcedure`（有具名 SID/STAR），不另发明一种 kind；若同一池里 STAR 与进近抢同一个代理点，以该条路径上胜出的程序类型为准，不能再假设「整次查询只有一种接法」。

## 7. 语义诚实：区分四种到达/离场连接

当程序无法（或不该）把机场写成 SID/STAR 接入时，回退原因不同，混为一谈会误导。引擎用 `ConnectionKind` 区分（Route 的 `dep_connection` / `arr_connection`）：

- `kProcedure`——确实用了 SID/STAR 接入（`sid` / `star` 有名），包括 §6.1 那种「代理点 + 航路串里写出入口」的拼接；
- `kTerminalTransition`——没有可用的 STAR（或与 STAR 拼接同台比价后进近胜出），走了 §6 的 DCT-to-IAF；`star` 为空，详情里带进近元数据；
- `kRadarVectors`——机场**发布了** SID/STAR，但没有任何衔接点能接到航路网（典型是雷达引导离场、没有定点出口）；
- `kDirect`——该侧根本没有可用的 SID / STAR / 进近衔接，纯 DCT 回退。

CLI / JSON 会显式标注「RADAR VECTORS」或「APCH PROC」等，把「雷达引导 / 终端过渡 / 缺数据」分开——这是语义诚实，而非假装有一条 STAR。判断依据是该侧是否发布了程序、以及本次是否真的用了 STAR 或进近。
...
## 9. 转弯角度软惩罚（TurnPenalty）

衔接模型允许「任意在网 fix、任意方向」接到程序公布的入口/出口。距离最优解有时会滥用这一点：SID 出口航向与下一段航路几乎反向，在交接 fix 上形成 **近掉头**——那不是一段可飞的转弯机动，而是连接点模型把几何不连贯的交接标成了「免费」。典型是 ZBSJ→ZGGG 一类在 OC 附近约 157° 的近掉头（issue #20）。

**修法不是「转弯贵」的运营定价**，而是给优化器一个软信号，让方向连贯的交接更便宜。实现是路径相关的 `TurnPenalty`（`astar.h`）：离开顶点时比较**入站航向**与**出站边航向**，把转角换成海里软代价加进 `g`。它**不是** `Constraint`——约束层只看单边，拿不到「怎么进来的」；因此惩罚写在 A\* 松弛循环里，Yen 的 `CostOfPath` 用同一公式重算，两处必须同步。

形态（分段，默认开启于 `FindRoutes`）：

- **0–45°**：零惩罚（中途航路转角绝大多数小于 15°，不扰动正常航路）；
- **45–90°**：线性升到 20 NM；
- **90–180°**：二次升到 **200 NM**（压近掉头；定档依据见 issue #20 / #21 复测——残留急转多数出在无程序的 DCT 回退机场，连接模型碰不到，200 仍有独立价值）。

实现取**贪婪单状态**：每个顶点只保留当前最优路径的入站航向，不把状态扩成 `(vertex, 航向桶)`。放弃严格全局最优（更长但更顺的入边可能被丢掉），换零状态膨胀与每松弛约一次 `atan2`；惩罚非负且启发仍只依赖顶点，在贪婪近似意义下仍 admissible/consistent。无程序航向时（`kNoBearing`）该步不加罚。

相关代码：`TurnPenalty` / `TurnAngleDeg`（`astar.h`）、A\* 松弛与 Yen 重定价。约束层边界见 [constraint-layer](constraint-layer.zh-CN.md)。
...
4. 到达/离场只在**航图公布的入口/出口**交接；若这些点接不进航路网，用 **DCT 拼接**把真正的衔接点写进航路串（#30），不再拿中段 fix 冒充；
5. 多源 K-shortest 让候选走不同程序；
6. 没有公布 STAR 时，用 **DCT-to-IAF** 接进近（航路串仍写 DCT，进近只进元数据）；有 STAR 但衔接点全不在航路上，见 §6.1；
7. 实在接不上时，**如实区分**程序接入 / 终端过渡 / 雷达引导 / 缺数据；
8. 衔接处用 **TurnPenalty** 软罚近掉头，避免连接点模型被滥用（§9）。
