...
这是软惩罚方案最容易被质疑的地方，值得说清。A\* 要求启发函数 h(v) 是**真实剩余代价的下界**才能保证最优。我们的 h 是单位球弦长（见 [routing-basics](routing-basics.zh-CN.md)），而软惩罚**增加**了边的有效代价——所以真实剩余代价只会变大，h 只会变得更保守，下界关系不受影响。
