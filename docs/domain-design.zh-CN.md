...
> BravoFinder 的领域类型为什么这么设计——不可变值类型、自研 `Result<T,E>`、无 `static`/无裸 `new`、以及数据驱动的紧凑内存表示（一度是 `SmallVec`，后为定长 `FixedIdent`）。这些不是零散的 风格偏好，而是 v2→v3 重写的核心动机的直接体现。面向想读源码、理解设计宪法的读者。相关代码： `libs/engine/core/domain/`、`libs/engine/core/result.h`、`libs/engine/core/base/small_vec.h`。
...
这几条合起来，让「一个只读实例多线程并发查」([线程安全契约](thread-safety.zh-CN.md))成为规矩的自然 推论，而不是事后补的锁。
